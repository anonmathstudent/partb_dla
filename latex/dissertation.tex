...
\subsection{Off-Lattice: Analytic Termination}
In the continuous model, aggregation occurs when the walker touches a cluster particle (distance $2r_p$). Simulating this directly is error-prone; a discrete time-step might overshoot the boundary or miss a ``grazing" collision.

To resolve this, we adopt the Exact Adhering method described by \citet{Bell2019}.
When the walker is close to a particle $P$ but also near other particles, we must determine whether it hits $P$ or escapes to a safer distance. We define a local domain $\Omega$ formed by the intersection of the ``capture circle" (the boundary of $P$) and a ``safe escape circle" (determined by the second-nearest neighbour).

This domain $\Omega$ forms a circular lune (a geometric shape bounded by two circular arcs). To sample the exit position from $\Omega$ analytically, we conformally map it to the upper half-plane in three stages:
\begin{enumerate}
    \item $f_1$: We translate and rotate the system to fix the particle centers.
    \item $f_2$: We map the two points where the circles intersect to $0$ and $\infty$. This transforms the curved boundaries of the lune into two straight rays emanating from the origin, forming a wedge of angle $\phi$.
    \item $f_3$: We apply a power map $z \mapsto z^{\pi/\phi}$ to open the wedge into the full upper half-plane.
\end{enumerate}
In this final transformed space, the complex exit problem reduces to a Brownian motion hitting the real line. We sample the hit point from the Cauchy distribution (derived in \cref{the:line_kernel}), and then apply the inverse maps $f_1^{-1} \circ f_2^{-1} \circ f_3^{-1}$ to find the exact collision or escape point on the original circles.
\begin{figure}[htbp]
    \centering
    \resizebox{\textwidth}{!}{% 
    \begin{tikzpicture}[
        node distance=0.8cm, % Gap between boxes
        >=Latex,
        thick,
        % Styles
        axes/.style={->, gray!60, thin},
        target boundary/.style={blue, very thick},          % The Cluster (q)
        escape boundary/.style={red!80!black, thick, dashed}, % The Reset Boundary (p)
        walker/.style={circle, fill=black, inner sep=1.5pt},
        domain fill/.style={fill=blue!5, opacity=0.8},      % Increased opacity slightly
    ...
    \node (stage1) [stage box] {
        \begin{tikzpicture}[scale=0.7]
            \coordinate (P) at (0, 0);   % p (Safe Center)
            \coordinate (Q) at (1.8, 0); % q (Target Center) - Adjusted for overlap
            \def\RSafe{2.2}
            \def\RTarget{1.5}

            % 1. Fill Valid Region (Safe minus Target)
            \begin{scope}
                \clip (P) circle (\RSafe); % Restrict everything to inside Safe Circle
                \fill[domain fill] (-3,-3) rectangle (3,3); % Fill background
                \fill[white] (Q) circle (\RTarget);         % Punch hole for Target
            \end{scope}
            ...
            % 3. Centers
            \fill[black] (P) circle (1.5pt) node[left, font=\scriptsize] {$p$};
            \fill[blue] (Q) circle (1.5pt) node[right, font=\scriptsize] {$q$};
            \draw[dashed, gray] (P) -- (Q);

            % 4. Labels
            \node[red!80!black, font=\tiny] at (-1.5, 1.5) {Escape};
            \node[blue, font=\tiny] at (2.5, 1.0) {Target};

            % Walker
            \node[walker, label={[above, font=\tiny]:$z$}] at (0.2, 0.2) {};
            
            \node at (0.9, -2.5) [align=center, font=\small] {\textbf{1. Physical Domain}};
        \end{tikzpicture}
    };

    %% --- STAGE 2: Centered & Rotated (f1) ---
    \node (stage2) [stage box, right=of stage1] {
        \begin{tikzpicture}[scale=0.7]
            \draw[axes] (-2.5,0) -- (2.5,0);
            \draw[axes] (0,-2.5) -- (0,2.5);
            
            % New Coordinates after f1: p -> 0, q -> Real Axis
            \coordinate (P) at (0,0);
            \coordinate (Q) at (1.8,0); % |p-q|
            \def\RSafe{2.2}
            \def\RTarget{1.5}
            ...
            \begin{scope}
                \clip (P) circle (\RSafe);
                \fill[domain fill] (-3,-3) rectangle (3,3);
            ...
            % Points w+ and w- (Intersections)
            % Visual approximation for diagram
            \fill[gray] (1.1, 1.9) circle (1.5pt) node[above, black, font=\tiny] {$w_+$};
            \fill[gray] (1.1, -1.9) circle (1.5pt) node[below, black, font=\tiny] {$w_-$};

            \node at (0, -2.5) [align=center, font=\small] {\textbf{2. Centered ($f_1$)}};
        \end{tikzpicture}
    };
    ...
    \node (stage3) [stage box, right=of stage2] {
        \begin{tikzpicture}[scale=0.7]
            ...
            % Wedge definition (target on bottom, safe on top usually)
            \def\WedgeAngle{60} 
            ...
            \node at (1.2, -2.5) [align=center, font=\small] {\textbf{3. Wedge ($f_2$)}};
        \end{tikzpicture}
    };
    ...
    \node (stage4) [stage box, right=of stage3] {
        \begin{tikzpicture}[scale=0.7]
            ...
            % Boundaries mapping to Real Line
            % Target -> Positive Real (Arbitrary choice for viz)
            \draw[target boundary] (0,0) -- (2.5,0);
            % Escape -> Negative Real
            \draw[escape boundary] (0,0) -- (-2.5,0);

            % Sample point
            \node[walker] at (0,1) {};
            \draw[->, dashed, gray] (0,1) -- (1.5,0) node[below, black, font=\tiny] {Hit $x$};

            \node at (0, -2.5) [align=center, font=\small] {\textbf{4. Half-Plane ($f_3$)}};
        \end{tikzpicture}
    };
...
To rigorously investigate the source of anisotropy in DLA, we introduce a hybrid model. This model combines \textbf{continuous transport} (Off-Lattice diffusion) with \textbf{discrete aggregation} (On-Lattice geometry).

\subsection{Motivation and Design}
We hypothesise that the anisotropy observed in lattice DLA arises from the aggregation step (the limited availability of attachment slots) rather than the transport step (the discrete random walk). To test this, we require a model where the walker moves via Brownian motion (isotropic) but aggregates onto a grid.
We considered three aggregation mechanisms:
\begin{enumerate}
    \item \textbf{Proximity Snap:} Aggregating if the walker enters a radius $\epsilon$ of a lattice point. This effectively turns lattice points into circular attractors, distorting the harmonic measure.
    \item \textbf{Antennae Method:} As proposed by \citet{Menshutin2011}, particles are only accepted if they hit the cluster at specific angles aligned with the axes. This introduces an artificial rejection probability that we wish to avoid.
    \item \textbf{Rotate and Snap:} Our chosen method. The walker diffuses continuously until it physically contacts the cluster (as in the Off-Lattice model). Upon contact, the particle slides along the surface of the hit particle to the nearest valid lattice coordinate.
\end{enumerate}

\subsection{Implementation}
The algorithm proceeds exactly as the Off-Lattice simulation (Walker on Spheres) until the collision condition $|z_{walker} - z_{cluster}| = 1$ is met.
Let the contact angle be $\phi$. We map this continuous angle to the discrete set of nearest neighbours $\{0, \pi/2, \pi, 3\pi/2\}$.
\begin{equation}
    \theta_{snap} = \frac{\pi}{2} \left\lfloor \frac{4\phi + \pi}{2\pi} \right\rfloor.
\end{equation}
The particle is then placed at $z_{new} = z_{cluster} + e^{i\theta_{snap}}$.

One might be concerned that the ``snapped'' position is already occupied. However, a simple geometric argument shows this is impossible. If the particle hits the cluster at angle $\phi$, the nearest lattice site is the one minimising $|\phi - \theta_{grid}|$. For the snapped site to be occupied, the walker would have had to pass through it to reach the current contact point, triggering an earlier collision. Thus, the nearest slot is always empty.


This hybrid approach allows us to maintain the conformal invariance of the incoming probability flux (via continuous diffusion) while forcing the final structure to conform to $\mathbb{Z}^2$ constraints.
...
% Notes: 3x3 neighbour logic vs vectors [cite: 88]
